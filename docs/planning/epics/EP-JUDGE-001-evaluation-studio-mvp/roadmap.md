...
| Slice 4D Dynamic Criteria Display | `WP-IN-2026-036 Judge Dynamic Criteria Display` | Done | Render all score criteria present in `CompareView` Judge results, including future/custom/saved EvaluationRun criteria. | Delivered by `IN-2026-036`; renderer-only compatibility change, no Judge runtime, IPC/storage/export, shared contract, package, or dependency changes. Manual smoke pending. |
| Slice 5 Tests/Smoke | `WP-JUDGE-008 Tests and Smoke Suite` | Done | Consolidate automated Judge coverage and manual smoke checklist across compatibility and Studio flows. | Delivered by `IN-2026-037`; created QA source-of-truth docs, evidence template, release confidence checklist, and docs completeness test. Manual product smoke remains a Human QA gate. |
...
## Next action
Run the consolidated Judge MVP smoke suite in `docs/qa/judge-mvp-smoke-suite.md` when an Electron session is available, using `docs/qa/judge-mvp-smoke-evidence-template.md` for evidence. After GO/NO-GO is recorded, continue with `WP-JUDGE-009 Research Comparison Mode` or the n8n EvaluationRun preflight slice.
